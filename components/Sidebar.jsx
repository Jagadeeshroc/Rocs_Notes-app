...
export default function Sidebar() {
  ...
  return (
    <aside 
      ...
      } bg-slate-950/80 overflow-visible`}
    >
      ...
      <div className="h-24 flex items-center px-6 overflow-hidden relative group">
         ...
         <div className={`absolute top-2 left-6 w-12 h-12 bg-cyan-500/20 rounded-full blur-xl transition-all duration-700 ${isWide ? "opacity-100" : "opacity-0"}`}></div>

        {/* Wide State Logo */}
        <div className={`flex items-center gap-4 transition-all duration-500 ${isWide ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}>
          <div className="relative">
             <FaStickyNote className="text-cyan-400 text-2xl drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]" />
          ...
          <h1 className="text-2xl font-bold tracking-[0.2em] text-white whitespace-nowrap font-sans">
            NOTES<span className="text-cyan-400">.AI</span>
          </h1>
        </div>
        ...
        {!isWide && (
           <div className="absolute left-0 w-full flex justify-center top-9">
              <FaStickyNote className="text-cyan-400 text-2xl transition-transform duration-300 group-hover:scale-110" />
           </div>
        )}
      </div>
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className={`absolute left-0 w-20 flex justify-center items-center h-full z-10 transition-all duration-300 ${isActive ? "drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]" : ""}`}>
                {item.icon}
              </div>
              
              {/* Text Label (Slides in/out) */}
              <span className={`font-medium tracking-wide whitespace-nowrap ml-16 pl-2 transition-all duration-500 ${
      ...
      <div className="p-4 border-t border-slate-800/50 backdrop-blur-md bg-slate-900/30">
        <div className={`flex items-center transition-all duration-500 ${isWide ? "justify-start" : "justify-center"}`}>
          ...
          <div className={`flex flex-col overflow-hidden transition-all duration-500 ${isWide ? "opacity-100 w-auto ml-4" : "opacity-0 w-0 ml-0"}`}>
            <span className="text-sm font-bold text-slate-200 truncate font-sans tracking-wide whitespace-nowrap">JAGADEESH</span>
            <span className="text-[10px] text-cyan-500 truncate tracking-[0.15em] uppercase whitespace-nowrap">System Admin</span>
          </div>
